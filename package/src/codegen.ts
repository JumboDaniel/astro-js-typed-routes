...
export function generateRoutes(config: AstroConfig): void {
  ...
  const dtsPath = join(generatedDir, "typed-routes.d.ts");
  const routesPath = join(generatedDir, "routes.gen.ts");
  ...
  console.log(`[astro-js-typesafe-routes] generated (${pages.length} routes)`);
}
